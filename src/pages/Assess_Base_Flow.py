...
from utils.base_flow.baseFlow_utils import baseFlow_main as bfm
...
if st.sidebar.button("Analyze Peak Flow Data"):
    ...
    st.write(f"Analyzing peak flow data for USGS Station ID: {usgs_station_id}, Begin Year: {begin_year}")
    df, trout_analysis, min_analysis = bfm(site_id=usgs_station_id, begin_year=begin_year)
    st.write("Trout Threshold Analysis:", trout_analysis)
    st.write("Minimum Threshold Analysis:", min_analysis)
    if df is not None:
        #st.dataframe(df)
    ...
    if not above_thresh_df.empty:
        ...
        st.write(f"##### Total number of days with flow below {pf_threshold} cfs:", len(above_thresh_df))
